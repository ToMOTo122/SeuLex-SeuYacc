...
fun_declaration	: fun_header LPAR parameters RPAR block
			{ g_fun_sym->type = types_fun(g_fun_ret_type, $3); }
		;

fun_header	: type NAME
			{ g_fun_ret_type = $1;
			  g_fun_sym = symtab_insert(scope, $2, 0);
			  scope = symtab_open(scope);
			  scope->function = g_fun_sym;
			}
		;
...
statement:	lexp ASSIGN exp SEMICOLON	{ check_assignment($1,$3); }
		| RETURN exp SEMICOLON
			{ check_assignment(g_fun_ret_type,$2); }
		| IF LPAR exp RPAR statement 		%prec LOW
