...
GrammarSpec parseGrammarFile(const std::string& filepath) {
    ...
    auto processRule = [&](const std::string& ruleText, int lhs) {
        ...
        for (const auto& alt : alternatives) {
            // empty alt = epsilon production, do not skip
            ...
            // Strip ALL {action} blocks from RHS; keep the last one as
            // the production semantic action. This correctly handles mid-rule
            // actions such as:
            //   fun_declaration : type NAME { mid } LPAR params RPAR { act } block
            {
                std::string rhs_clean;
                std::string last_action;
                size_t idx = 0;
                while (idx < rhsStr.size()) {
                    if (rhsStr[idx] == '{') {
                        int depth = 0;
                        size_t blk_start = idx;
                        while (idx < rhsStr.size()) {
                            if (rhsStr[idx] == '{') depth++;
                            else if (rhsStr[idx] == '}') {
                                depth--;
                        ...
                        last_action = rhsStr.substr(blk_start + 1, idx - blk_start - 2);
                    } else {
                        rhs_clean += rhsStr[idx];
                        idx++;
                    }
                }
                rhsStr = trim(rhs_clean);
                action = last_action;
            }
